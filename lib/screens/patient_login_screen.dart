  ...
  Future<void> _loginWithEmail() async {
    ...
    if (result['success'] == true) {
      ...
      if (user?.isBlocked == true) {
        await auth.logout();
        _showError("Your account is blocked.");
      } else if (user?.isApproved == false) {
        await auth.logout();
        _showError("Account not approved yet.");
      } else if (user?.role.name == 'patient') {
        Navigator.pushReplacementNamed(
          context,
          PatientDashboard.routeName,
        );
      } else {
        await auth.logout();
        _showError("Not a patient account.");
      }
    } else {
